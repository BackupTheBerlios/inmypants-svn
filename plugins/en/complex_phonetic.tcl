...
# IMP_plugin_complex_phonetic_xform procedure
...
proc IMP_plugin_complex_phonetic_xform { text style } {
	...
	foreach letter $letters {
		if { [ regexp -nocase {[A-Za-z0-9]} $letter ] } {
			if { $style == 0 } {
				...
				IMP_putloglev d * "IMP: (phonetic) did not get a style that makes sense"
				return ""
			}
		} elseif { $letter == " " } {
			set line "$line "
...
# end IMP_plugin_complex_phonetic_xform

# IMP_plugin_complex_phonetic procedure
# callback for the imp plugin
proc IMP_plugin_complex_phonetic { nick host handle channel text } {
	# JamesOff says that if I don't do this, then IMP will start ignoring
	...
	# phonetic the line
	set line [ IMP_plugin_complex_phonetic_xform $text $style ]

	# print out the reply
	if { $line != "" } {
		puthelp "PRIVMSG [chandname2name $channel] :$line"
		IMP_putloglev d * "IMP: (phonetic:$command) $nick needed it spelled out, so I did"
	}
}
# end IMP_plugin_complex_phonetic

#  IMP_plugin_output_asshat procedure
# make the phonetic asshat plugin a general output filter
proc IMP_plugin_output_asshat { channel text } { 
	return [ IMP_plugin_complex_phonetic_xform $text 1 ]
}
# end IMP_plugin_output_asshat

#  IMP_plugin_output_phonetic procedure
# make the phonetic plugin a general output filter
proc IMP_plugin_output_phonetic { channel text } { 
	return [ IMP_plugin_complex_phonetic_xform $text 0 ]
}
# end IMP_plugin_output_phonetic


# register callbacks
IMP_plugin_add_complex "phonetic" "^!phonetic|^!asshat" 100 "IMP_plugin_complex_phonetic" "en"
IMP_plugin_add_output "asshat" IMP_plugin_output_asshat 0 "en"
IMP_plugin_add_output "phonetic" IMP_plugin_output_phonetic 0 "en"
