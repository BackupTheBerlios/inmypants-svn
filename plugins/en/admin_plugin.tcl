# IMP: admin plugin file for plugin mangement
...
#register the plugin
IMP_plugin_add_admin "plugin" "^plugin" n "IMP_plugin_admin_plugins" "any"

proc IMP_plugin_admin_plugins { handle idx { arg "" }} {
  #global IMP_plugins_admin IMP_plugins_simple
 
  #plugin remove <type> <id>
  if [regexp -nocase {remove ([^ ]+) (.+)} $arg matches t id] {
    putidx $idx "Removing $t plugin $id...\r"
    set full_array_name_for_upvar "IMP_plugins_$t"
  ...
  if [regexp -nocase {enable ([^ ]+) (.+)} $arg matches t id] {
    if {$t == "output"} {
      ...
      global IMP_plugins_output
      set details $IMP_plugins_output($id)
      ...
      set IMP_plugins_output($id) "$callback¦1¦$language"
      putlog "IMP: INFO: Output plugin $id ($language) enabled"
      return 0
    }
  ...
  if [regexp -nocase {disable ([^ ]+) (.+)} $arg matches t id] {
    if {$t == "output"} {
      ...
      set IMP_plugins_output($id) "$callback¦0¦$language"
      putlog "IMP: INFO: Output plugin $id disabled"
      return 0
    }
  ...
  putidx $idx "Installed IMP plugins:\r"
