...
source "$IMPModules/leet_settings.tcl"

# Binds
bind pub - "!leet" IMPLeetChannel
bind msg - leet leetPrivate

proc IMPLeetChannel {nick host handle channel text} {
  global botnick IMPInfo
  if {$IMPInfo(balefire) != 1} {
    return 0
  }
...
proc leetPrivate {nick host handle arg} {
  ...
  if [regexp {(\[#!\]\w+) (.+)} $arg ming channel param] {
    ...
    putlog "IMP: $nick asked !$param! to be leeted to !$channel!"
    if {![botonchan $channel]} {
      puthelp "PRIVMSG $nick :Sorry, I'm not on $channel"
      return 0
    }
    if {![onchan $nick $channel]} {
      set name [IMPTransformNick $nick $nick $host]
      puthelp "PRIVMSG $nick :I'm sorry $name, I can't do that."
      putlog "IMP: ALERT! $nick failed query leet to $channel ($param)."
      return 0
    }
  ...
  putlog "IMP: $nick asked !$arg! to be leeted"
